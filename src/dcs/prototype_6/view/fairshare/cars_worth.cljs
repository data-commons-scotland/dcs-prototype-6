              ...
              [:div.content.is-small.has-text-info
               [:ol.is-lower-alpha
                [:li "The Fair Share have calculated their CO" [:sub "2"] "e values using "
                 [:a {:href "https://ghgprotocol.org/Third-Party-Databases/Defra" :target "_blank"} "Defra"] " formulae."]
                [:li "A rough approximation based on figures from "
                 [:a {:href "https://www.brusselsblog.co.uk/carbon-emissions-in-the-lifetimes-of-cars/" :target "_blank"} "Carbon emissions in the lifetimes of cars"]
                 " and "
